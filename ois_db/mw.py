...
from_datetime_str = '2021-8-1'
to_datetime_str = '2021-10-31 23:00'
...
ss_max_mw_df = ss_df.join(transformer_33kv_max_df).sort_values(by=['ss'], ignore_index=True)
...
# df = px.data.tips()
fig = px.histogram(ss_max_mw_df_value_only, y="ss", x='MW')
fig.update_layout(
        title=f"Substation Max Load between {from_datetime_str} and {to_datetime_str}",
